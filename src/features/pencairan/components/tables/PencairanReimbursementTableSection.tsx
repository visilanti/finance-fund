...
interface PencairanReimbursementTableSectionProps {
  initialData?: PencairanItem[];
}

export function PencairanReimbursementTableSection({ initialData = [] }: PencairanReimbursementTableSectionProps) {
  const {
    filteredData,
    isLoading,
    isSubmitting,
    ...
    loadData,
  } = usePencairan({ initialData, jenis: "Reimbursement" });
  ...
  const columns = useMemo<ColumnDef<PencairanItem>[]>(
    () => [
      ...
      {
        key: "kegiatan",
        header: "Keterangan Usulan Reimbursement",
        width: "auto",
        cell: (item) => (
  ...
  return (
    <>
      <MasterDataTableShell<PencairanItem>
        ...
        toolbarProps={{
          searchPlaceholder: "Cari di Pencairan Reimbursement...",
          filterOptionGroups: [
            {
              id: "status",
          ...
          onExport: () => alert("Mengekspor data Pencairan Reimbursement..."),
          onRefresh: loadData,
        }}
      />
