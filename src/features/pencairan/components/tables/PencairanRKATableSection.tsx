...
interface PencairanRKATableSectionProps {
  initialData?: PencairanItem[];
}

export function PencairanRKATableSection({ initialData = [] }: PencairanRKATableSectionProps) {
  const {
    filteredData,
    isLoading,
    isSubmitting,
    ...
    loadData,
  } = usePencairan({ initialData, jenis: "RKA" });
  ...
  return (
    <>
      <MasterDataTableShell<PencairanItem>
        ...
        toolbarProps={{
          searchPlaceholder: "Cari di Pencairan RKA...",
          filterOptionGroups: [
            {
              id: "status",
          ...
          onExport: () => alert("Mengekspor data Pencairan RKA..."),
          onRefresh: loadData,
        }}
      />

      {/* Drawer Detail Pencairan Terpisah */}
      ...
      {/* Modal Aksi Upload Bukti Transfer */}
