...
interface FormItemsRKATableProps {
...
export function FormItemsRKATable({
  items,
  onAddItem,
  onRemoveItem,
  onUpdateItem,
      ...
      <div className="overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0">
        <table className="w-full text-left border-collapse min-w-[680px]">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
            {items.map((item, idx) => {
              const selectedKelompok = dataRKA.find((k) => k.namaKelompok === item.kelompok);
              const kegiatanOptions = selectedKelompok
                ? selectedKelompok.DetailItemRKA.map((d) => d.kegiatanRka)
                : Array.from(new Set(dataRKA.flatMap((k) => k.DetailItemRKA.map((d) => d.kegiatanRka))));

              const selectedKegiatan =
                selectedKelompok?.DetailItemRKA.find((d) => d.kegiatanRka === item.kegiatanRka) ||
                dataRKA.flatMap((k) => k.DetailItemRKA).find((d) => d.kegiatanRka === item.kegiatanRka);

              const bulanOptions = selectedKegiatan ? selectedKegiatan.detail.map((d) => d.bulan) : [];

              const handleKelompokChange = (val: any) => {
                const strVal = typeof val === "string" ? val : val?.target?.value || "";
                onUpdateItem(idx, "kelompok", strVal);
                const targetKel = dataRKA.find((k) => k.namaKelompok === strVal);
                const stillValid = targetKel?.DetailItemRKA.some((d) => d.kegiatanRka === item.kegiatanRka);
                if (!stillValid && item.kegiatanRka) {
                  onUpdateItem(idx, "kegiatanRka", "");
                  onUpdateItem(idx, "bulan", "");
                  onUpdateItem(idx, "budgetRka", 0);
                }
              };

              const handleKegiatanChange = (val: any) => {
                const strVal = typeof val === "string" ? val : val?.target?.value || "";
                onUpdateItem(idx, "kegiatanRka", strVal);
                const targetKel = dataRKA.find((k) => k.namaKelompok === item.kelompok);
                const parentKel = item.kelompok
                  ? targetKel
                  : dataRKA.find((k) => k.DetailItemRKA.some((d) => d.kegiatanRka === strVal));
                const matched = parentKel?.DetailItemRKA.find((d) => d.kegiatanRka === strVal);

                if (matched && parentKel) {
                  if (!item.kelompok) {
                    onUpdateItem(idx, "kelompok", parentKel.namaKelompok);
                  }
                  const availableMonths = matched.detail.map((d) => d.bulan);
                  let targetMonth = item.bulan;
                  if (!targetMonth || !availableMonths.includes(targetMonth)) {
                    targetMonth = availableMonths[0] || "";
                    onUpdateItem(idx, "bulan", targetMonth);
                  }
                  const matchedDetail = matched.detail.find((d) => d.bulan === targetMonth);
                  onUpdateItem(idx, "budgetRka", matchedDetail?.budget || 0);
                } else {
                  onUpdateItem(idx, "bulan", "");
                  onUpdateItem(idx, "budgetRka", 0);
                }
              ...
              const handleBulanChange = (val: any) => {
                const strVal = typeof val === "string" ? val : val?.target?.value || "";
                onUpdateItem(idx, "bulan", strVal);
                if (selectedKegiatan) {
                  const matchedDetail = selectedKegiatan.detail.find((d) => d.bulan === strVal);
                  if (matchedDetail) {
                    onUpdateItem(idx, "budgetRka", matchedDetail.budget);
                  }
                }
              };
                ...
                <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="px-3 py-2.5">
                    <Select
                      ...
                      placeholder={item.kegiatanRka ? "Pilih Bulan..." : "Pilih Kegiatan..."}
                      disabled={!item.kegiatanRka || bulanOptions.length === 0}
                      isSearchable={false}
                    />
                  </td>
